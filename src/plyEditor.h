...
{
public:
	void setup(string _name, string filepath, int gui_x, int gui_y)
	{
		this->name = _name;
		loadBtn.addListener(this, &plyEditor::loadPLY);
		writeBtn.addListener(this, &plyEditor::exportPLY);
		gui.setup(_name, ofxPanelDefaultFilename, gui_x, gui_y);
...
private:
	...
	//	editor.save(ofGetTimestampString()+".ply", hue, saturation, brightness);
		string filename = name + "_hue_"+ofToString(float(hue)) + ".satu_" + ofToString(float(saturation)) + ".bri_" + ofToString(float(brightness)) + ".ply";
		save(filename, hue, saturation, brightness);
	}
